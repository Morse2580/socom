...
def _triage_rank(root: Path, eligible: list, focus=None):
    """Rank the eligible bucket by recovery-worth, highest first. Worth = relevance of
    each promise's meaningful intent to a FOCUS (an explicit query, else the active-lesson
    corpus), computed by REUSING l0_score (the query keyword floor — no new scorer). The
    tie-break (fewer attempts, more-recent death, run-id) is applied as a STABLE pre-sort,
    so equal-worth promises keep that order. Degrades LOUDLY: when no candidate overlaps
    the focus at all, returns the tie-break order with a 'recency' basis rather than a
    silently arbitrary one. Returns (ranked_entries_with_worth, basis_string)."""
    ...
    if focus:
        focus_text, basis = focus, f"focus: {focus}"
    else:
        focus_text, basis = _active_lessons_text(root), "earned lessons"
    ...
    if not any(x["worth"] for x in enriched):
        # no relevance signal — fall back to the tie-break order, and SAY so (R6).
        return enriched, "recency (no relevance signal)"
    ranked_ids = l0_score(focus_text, [{"id": x["promise"], "text": x["_intent"]}
                                       for x in enriched], len(enriched))
    by = {x["promise"]: x for x in enriched}
    return [by[i] for i in ranked_ids], basis
...
def _cmd_triage(root: Path, args: list):
    ...
    now = datetime.now(timezone.utc)
    print(f"  {'#':<3} {'promise':<16} {'worth':<6} {'attempts':<9} {'age':<7} run-id")
    for n, x in enumerate(ranked, 1):
        d = x["dead"]
        print(f"  {n:<3} {x['promise']:<16} {x['worth']:<6} "
