...
| 6f — **verify→ledger ✓ DONE**, rest open | Contract machinery followups. **Shipped** (commit 927905d): `socom contract verify --record` — the FIRST automatic run-ledger producer. A fully-mechanical verify of a promise appends one `.socom/ledger/runs.jsonl` row (pure `_ledger_row`/`_next_attempt`, 9-key wire contract, `socom cycle` round-trips it), so cycle/lesson begin measuring REAL pass@1/pass@k instead of synthetic fixtures. Fail-closed (§separation-of-privilege): standalone contract / manual-pending / zero-auto-check all WITHHELD — the mechanical runner never records a verdict it cannot claim. Opt-in (verify is the runner; auto-recording would flood the ledger). **Rest still open** (#6f-2): standalone `.socom/contracts/` storage + lifecycle, §R10 renegotiation, per-contract pass@1/pass@k in cmd_cycle, wire `--record` into the task-completion gate / closeout so the ledger fills without the manual flag. | `contracts/` | **`socom contract verify --record`** | the ledger auto-append gap is half-closed: verify now feeds real data |
