...
| **7 — NEXT** | commit-protocol → canon (or socom.yaml) | `docs/COMMIT-PROTOCOL.md` | structured-commits principle + commit-msg gate | per-block semantics/templates (the #5 deferred half) |
| 6f | Contract machinery followups | `contracts/` | `socom contract verify` (the runner) | standalone storage, §R10 renegotiation, assertion/ledger recording, per-contract cycle metrics |
